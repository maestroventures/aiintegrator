...
- **A landed action (a draft, a staged invite, a written CRM field the operator must approve)** → confirm it actually reached the **live tool the operator works in** — open the real draft, read the real field — not parked in a document that *describes* the action. A finished action sitting in a debrief or a copy-paste block is a silent failure (Core §8, Land It or Flag It). This is the automated check behind that standard.
