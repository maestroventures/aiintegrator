...
/* prove-hosted-handoff.js — added 2026-09-17 (card
   ...
   byte-identical in both skill folders. It goes RED if any of these regress:
     ...
     H6  CLOUD without --folder is refused and writes nothing;
...
function build(flags, env) {
  ...
  const r = spawnSync(process.execPath, [BUILDER, docPath, configPath, out].concat(extra, flags || []),
...
try {
  ...
  /* H6, H7 */
  const noFolder = build(['--cloud']);
  check('H6 cloud without --folder is refused and writes nothing',
    noFolder.rc !== 0 && /needs --folder/.test(noFolder.stderr) && nothingWritten(noFolder), noFolder.rc + ' ' + noFolder.stderr);
