...
function buildBodyHtml(d) {
  ...
  /* follow-ups — CUT (b), 2026-07-30. An EMAIL follow-up is already a real Gmail draft
     ...
     words twice is the thing the operator cut. Channels with no other home (Text, Call) still carry
...
function captureBlock(id, placeholder) {
  return '<div class="capture"><div class="capture-label">✍️ Your notes</div>' +
    '<div class="cgb-tawrap"><textarea class="note" id="dbta-' + esc(id) + '" data-note="' + esc(id) + '" placeholder="' + esc(placeholder || 'Notes — type, or talk, hit the mic') + '"></textarea>' +
    '<button class="cgb-mic" id="dbmic-' + esc(id) + '" onclick="warmDictate(\'dbta-' + esc(id) + '\',\'dbmic-' + esc(id) + '\')" title="Talk instead of type">\uD83C\uDFA4</button></div></div>';
}
...
/* ════════════════════════════════════════════════════════════════
   ...
   3 of 15 debriefs shipped with no question panel at all — three real debriefs
   (2026-08-10, 2026-08-06 and 2026-07-30). The operator found it by
  ...
  const cards = qs.map((q, i) => {
    ...
    return '<div class="cq" id="cq' + i + '" data-idx="' + i + '"' +
      ...
      '<div class="cq-other" id="cqother' + i + '">' +
        '<div class="cgb-tawrap"><input class="note" id="cqothertxt' + i + '" placeholder="In your own words — what set it off? Type, or talk.">' +
        '<button class="cgb-mic" id="cqothermic' + i + '" onclick="warmDictate(\'cqothertxt' + i + '\',\'cqothermic' + i + '\')" title="Talk instead of type">\uD83C\uDFA4</button></div>' +
...
/* ── runtime ── */
/* ── THE MICROPHONE, CARRIED ACROSS — NOT A SECOND CONTROL (2026-09-17). ──────────────
   dr_anywhere_you_can_type_you_can_speak_20260917, the operator verbatim: "anywhere there is an
   input box, the user can type, or there needs to be a microphone that allows the user to
   speak like I am to you right now... anywhere in a call guide, anywhere in a modal."
   A debrief drew SIX typed boxes and none of them could be spoken into — the box a person
   most wants to talk into is the one right after the call. This is the SAME text as the
   `warmDictate` row the guide builder carries, copied across rather than rewritten: a
   second spelling of a registered control is drift wearing a helpful face.
   prove-debrief-speakable.js measures the BUILT PAGE, because counting source boxes is
   what hid this (three of the source boxes reach no page at all). */
const WARM_DICTATE_RUNTIME =
...
'<div class="ov" id="lcnOv"><div class="ov-card"><h2>Log Debrief</h2><p class="lcn-sub" id="lcnWho">Debrief —</p>' +
  ...
  '<div class="cgb-tawrap"><textarea id="lcnNotes" class="lcn-ta" placeholder="Type, or talk — hit the mic."></textarea>' +
  '<button class="cgb-mic" id="lcnNotesMic" onclick="warmDictate(\'lcnNotes\',\'lcnNotesMic\')" title="Talk instead of type">\uD83C\uDFA4</button></div>' +
...
'<script>' + WARM_DICTATE_RUNTIME + STANDALONE_RUNTIME + CQ_RUNTIME + '</' + 'script>',
'</body></html>'
  ].join('\n');
}
