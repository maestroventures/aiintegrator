...
**`<skill-folder>` means the INSTALLED plugin's `aii-call-guide` folder** (added 2026-09-25): the pack
bundles `build-call-guide.js`, `register-call-doc.js` and `upload-call-doc.js` there side by side. A
source copy of the builder does NOT have the other two beside it — on the build machine they live in
the workspace's `04 — Daily Operating System/scripts/`. So resolve each of the three separately from
the path map, and confirm the file exists before you run it; a missing script is a STOP, not a skip.
...
2. **Write (or rewrite) one `platform_picture_talk` row per step** for that link (key: tenant,
   ...
   know has changed since `from_telling` / `set_at` — **and when the purpose of THIS call has
   changed.** Re-read every existing row against this call's purpose and what they have already
   seen: on a second call, *"before we talk about you, let me show you ours"* is stale. Rewrite any
   line that no longer fits; keep only the rows that still do.
...
**No invite yet (added 2026-09-25).** When the call is agreed but not booked, there is no calendar
id — don't stall and don't invent one that looks real. Use `eventId` =
`tbd-<person-slug>-<purpose-slug>-wk<YYYYMMDD>` (the Monday of the expected week), set
`"eventIdSource":"operator"` in `config.json`, and `meetingDate` = the best-known date. **Re-key it
when the invite lands:** point the `call_doc` row and the pointer (Step 4.5) at the real calendar id,
with `event_id_source` `calendar`.
...
```
   ...
   **The health badge (0.9.53).** The builder draws a badge at the top of the guide from the email lookup you hand it. Green reads "Checked — current as of <newest email>". If the mailbox holds a newer email than the CRM, it reads "The CRM is missing your newest email with this person". When you counted the gap, add `"crmMissing": <n>` beside `crm` and `mailbox` in `newestEmail`, and the badge says "The CRM is missing <n> email(s) with this person". If you know of a problem the builder cannot see (for example, the meeting time changed after you read the calendar), add `"health": {"problems": [{"tone": "amber" | "red", "say": "<one plain sentence>"}]}` to the guide JSON. Write the sentence the way you would say it to the person who took the call. A sentence with internal words (an underscore, a table name, an exit code) refuses the build with exit 23. Every guide also carries a "Something wrong? Report it" button. You do nothing for it.
   **Something wrong? Report it:** on every page, in every color. Opened from the Command Center, one click sends the report. Opened from a file, it copies "Report: <kind> <doc_id> for <person> <date> — <reasons> — <note>" plus a `[call-doc-report {...}]` line. When a user pastes that into a session, file it with `call_doc_report_put('<tenant>', '<the JSON after call-doc-report>'::jsonb, 'pasted', 'session:<your session id>')` through the board connector (resolve it by category). Then tell them in one line what the store said back.
