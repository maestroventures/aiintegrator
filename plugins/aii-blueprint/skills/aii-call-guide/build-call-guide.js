...
/* ── LINKS + DOCUMENTS (2026-09-17) ─────────────────────────────────────────────────────
   ...
   falls back to what the build ALREADY carries in config (company domain, CRM lead), and renders
   the documents the guide tells the operator to open or screen-share:
     g.links.docs       [{label,url,note}]   — guide-wide documents (pre-call reading)
     section.docs       [{label,url,note}]   — documents one talking point uses; rendered at the
...
function buildLinksHtml(g,config){var L=(g&&g.links)||{};var C=config||{};var p=L.person||{},c=L.company||{};var chips=[];
  ...
  return html;}
function cgDocsBar(d){var ds=docsOf(d);return ds.length?'<div class="linkbar cgb-docs">'+ds.map(docChip).join('')+'</div>':'';}
...
function buildSectionsHtml(g) {
  ...
  const linksInner = buildLinksHtml({ links: g.links }, null);
...
function buildBoardHtml(g, config) {
  const refs = cgRefItems(g, config);
...
function cgCardHtml(item) {
  ...
  return '<div class="card cgb-card" id="c-' + esc(item.sid) + '">' +
    ...
    cgDocsBar(d) + '<div class="scard cgb-words">' + words + '</div>' + capture +
...
function cgRefItems(g, config) {
  ...
  push('links',  'Links',       'person · company · documents', buildLinksHtml(g, config),
...
/* ── --folder — added 2026-09-17 (card
   ...
   cloud mode, cloud mode without --folder, a missing or unreadable file. An UNRESOLVED answer
   is not a refusal here — it becomes the named handoff status folder_address_unresolved. */
function liftFolder(argv, filing) {
  ...
  if (!folderPath) {
    throw new Error('FILING MODE REFUSED - cloud mode needs --folder <rows.json>: the saved answer of\n' +
      '  node register-call-doc.js --folder-sql --channel <c> --company <x> [--partner <p>]\n' +
      'run through the board connector ({"error": "..."} if it raised). A cloud document is filed into a\n' +
      'REGISTERED Calls folder or it is not registered at all. Nothing was built.');
  }
...
async function main() {
  ...
  /* --folder is lifted out here, beside --cloud, for the same reason: the positionals must
     never shift. Added 2026-09-17 (hosted handoff — see register-call-doc.js). Cloud mode
     REQUIRES it: the Calls folder is resolved BEFORE the row exists, so a company with no
     registered folder never gets a row that can only say "filing pending" forever. */
  ...
  if (filing.cloud) {
    ...
    out.hosted = handoff;               // 2026-09-17: every remaining step, machine-readable
  }
  console.log(JSON.stringify(out, null, 2));
}
