...
**WHAT THE CREATE DOOR REFUSES — and until 2026-09-01 it refused the wrong one.** The CLAIM refuses
the wrong pairing at run time (above). The door that CREATES a job refuses three things at write
time, and none of them fires on a true sentence:

- **An unstated `reach` is refused.** It used to default silently to `tenant-store-only`, so an author
  who never once considered the question got the permissive answer written down as though it had been
  decided. An unstated value must never read as a stated one.
- **A `tenant-store-only` claim contradicted by the job's own body is refused.** A body address is
  either `sql:<object>` — in the tenant store, readable by any awake executor — or a PATH, which is a
  file on the operator's own machine (Step 6). A row asserting both is asserting two things that
  cannot both be true. This is a structural contradiction read off the row, never an inference about
  anyone's intent.
- **An `operator-files` job that names no way out is refused.** The value is legal and true; it is not
  permanent. The note must carry a line beginning `HOSTED ROUTE OWED:` naming the hosted door that
  would let this job drop to `tenant-store-only`, or saying plainly that none is known and why. Either
  is acceptable. Silence is not — an undated exception is how a retiring value becomes the status quo.

⚠ **THIS REPLACED A REFUSAL AIMED AT THE TRUTHFUL VALUE. The old design is described rather than
deleted, because it is the intuitive one and the next reader will reach for it.** The create door used
to refuse `reach = operator-files` outright, enforcing an instance rule that every job whose data lives
on the operator's machine must reach a hosted door or be retired. **That rule was never wrong. Its
enforcement was aimed at the sentence rather than the situation** — and a sentence nobody may write
does not stop the situation, it just stops anyone describing it.

Measured on one estate, in one session, in both directions: the truthful `operator-files` write was
REFUSED; a `tenant-store-only` claim on a job whose own body lived on the operator's machine was
ACCEPTED; and passing no `reach` at all was ACCEPTED and written as `tenant-store-only`. **The only
reach wall in that estate fired on the honest answer and waved through the false one** — so the only
route to creating an honestly machine-bound job was to lie about it, and a lie here is precisely the
false green named two paragraphs above. Ruled by the operator, verbatim: *"refuse the lie, not the
truth."*

⚠ **HONEST LIMIT, AND READ IT BEFORE YOU RELY ON THESE THREE REFUSALS.** They are DESCRIBED here as
the discipline every seat should hold, and they are ENFORCED wherever the create door has been brought
up to it. As of 2026-09-01 that is the estate the ruling was made on and not yet every client store, so
on some seats this section is the rule and the door does not yet hold it. **Check your own door before
you tell anyone it refuses these** — a rule quoted as enforced where it is only written is the same
words-and-door gap this whole block exists to close, one layer out.
